...
Poly_Fork = {
  ...
  new = function (self)
    pf = {
      target = "engine",
      -- target = {type="engine"}
      -- target = {type="crow", device=1}
      -- target = {type="midi", device=1, mode="poly", channels={1}}
      port = 1,
      mode = "poly",
      channels = {CH(1)},
      ...
      note_on = function (self,note,vel)
        ...
        elseif self.target=="crow" then
          crow.output[1].volts = util.linlin(0,120,-5,5,note) -- cv
          crow.output[2].volts = 5 -- gate
        end
