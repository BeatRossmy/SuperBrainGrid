...
GraphTheory = {
  ...
  VERTEX = function (_id,conf)
    return {
      ...
      edges = conf and conf["edges"] or {(_id==64 and 1 or _id+1)},
      ...
      tick = function (self,vertices,out)
        ...
        if #self.vertex.notes>0 and self.active then
          for _,n in pairs(self.vertex.notes) do
            out:note(n+12*self.octave,0.6,clock.get_beat_sec()*GraphTheory.speeds[self.speed]*0.95)
          end
        end
      end,
  ...
  new = function (engine_state,_p)
    ...
    gt.grid_event = function (self,e)
      --local i = e.x+(e.y-1)*16
      local i = m_index(e,1,1,16,4)
      ...
      if self.view=="speed" then
        if e.type=="click" then
          ...
          if m_index(e,1,e.y,8,1) then
            local i = m_index(e,1,e.y,8,1)
            self.playheads[e.y].speed = i
            BRAIN:set_overlay("speed",GraphTheory.speeds[i])
          elseif m_index(e,10,e.y,3,1) then
            local i = m_index(e,10,e.y,3,1)
            self.playheads[e.y].octave = GraphTheory.octaves[i]
            BRAIN:set_overlay("octave",GraphTheory.octaves[i])
          end
        end
      end
      ...
      if self.selected_playhead then
        ...
        elseif e.type=="hold" and e.x<17 then
    ...
    gt.redraw = function (self, buf)
      
      if self.view == "speed" then
        for _,p in pairs(self.playheads) do
          v_radio(buf,1,#GraphTheory.speeds,p.id,p.speed,2,10)
          ui_v_radio(buf,10,p.id,GraphTheory.octaves,p.octave)
        end
        return
      end
      ...
      for _,v in pairs(self.vertices) do
        if #v.notes>0 then 
          ...
          local pos = i_matrix(v.id,1,1,16,4)
          buf:led_level_set(pos.x,pos.y,2)
        end
      end
      
      -- playhead
      for i,p in pairs(self.playheads) do
        buf:led_level_set(16,4+p.id,p.active and 5 or 2)
        if p.active then
          local step = p.vertex and p.vertex.id or 1
          --local pos = index_to_pos[step]
          pos = i_matrix(step,1,1,16,4)
          ...
          buf:led_level_set(pos.x,pos.y,level,style)
        end
      ...
      if self.selected_playhead then
        ...
        if p.start then
          ...
          local pos = i_matrix(p.start,1,1,16,4)
          buf:led_level_set(pos.x,pos.y,15)
        end
      end
      ...
      if self.selected_vertex then
        ...
        local pos = i_matrix(self.selected_vertex,1,1,16,4)
        buf:led_level_set(pos.x,pos.y,15)
        for _,e in pairs(self.vertices[self.selected_vertex].edges) do
          --local pos = index_to_pos[e]
          local pos = i_matrix(e,1,1,16,4)
          buf:led_level_set(pos.x,pos.y,15,"fade")
        end
      end
